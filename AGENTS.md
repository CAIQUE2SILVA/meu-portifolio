...
- **Markdown for Agents:** `npm run build` generates `www/portfolio.md`. Requests with `Accept: text/markdown` are served markdown via the Cloudflare Worker (`worker/index.ts`) and the Netlify Edge Function (`netlify/edge-functions/markdown.ts`). HTML remains the default for browsers.

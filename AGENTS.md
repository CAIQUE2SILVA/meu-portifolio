...
- **Agent discovery:** `npm run build` also generates `/.well-known/*`, `auth.md`, and `_headers` (see `scripts/generate-agent-discovery.mjs`). DNS-AID records require DNS provider configuration — see `docs/DNS-AID.md`.
